...
function ListingsPage() {
    ...
    useEffect(() => {
        ...
        listings.forEach(listing => fetchEvaluation(listing.id));
    }, [listings.length]);

    const fetchEvaluation = async (listingId) => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`http://localhost:8080/evaluation/simple/${listingId}`, {
                headers: { "Authorization": `Bearer ${token}` },
            });
            if (response.ok) {
                const data = await response.json();
                setEvaluations(prev => ({ ...prev, [listingId]: data }));
            }
        } catch (err) {
        ...
        <div className="min-h-screen bg-[#0f172a] p-8">
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-[#64748b] text-sm mt-1">{listings.length} properties found</p>
                </div>
                <button
                    ...
                    >
                        ...
                        <div className="relative h-48 bg-[#0f172a] flex items-center justify-center overflow-hidden">
                            {listing.primaryImageUrl
                                ? <>
                                    <img src={`http://localhost:8080${listing.primaryImageUrl}`} alt={listing.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            ...
                            <span className={`absolute top-3 right-3 text-xs font-bold px-2.5 py-1 rounded-full border backdrop-blur-sm ${
                        ...
                        <div className="p-5">
                            ...
                            <p className="text-[#94a3b8] text-sm mb-3">
                                {propertyTypes[listing.propertyType] || listing.propertyType} · {listing.sizeM2}m² {listing.bedroomCount ? `· ${listing.bedroomCount} bd` : ""}
                            </p>
                            <p className="text-amber-400 font-bold text-xl mb-4">₺{listing.price.toLocaleString()}</p>

                            {/* Evaluation */}
                            {evaluations[listing.id] && (
                                <div className="pt-4 border-t border-[#334155]">
                                    <div className="flex items-center justify-between mb-3">
                                        <span className="text-xs font-semibold uppercase tracking-widest text-[#64748b]">Eval Score</span>
                                        <div className="flex items-center gap-2">
                                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${verdictColor(evaluations[listing.id].verdict)}`}>
                                                {evaluations[listing.id].verdict}
                                            </span>
                                            <span className="text-sm font-bold text-white">
                                                {evaluations[listing.id].totalScore}/{evaluations[listing.id].maxTotalScore}
                                            </span>
                                        </div>
                                    </div>
                                    {evaluations[listing.id].categoryScores.map(cat => (
                                        <div key={cat.categoryName} className="mb-2">
                                            <div className="flex justify-between text-xs mb-1">
                                                <span className="text-[#94a3b8]">{cat.categoryName}</span>
                                                <span className="text-white font-medium">{cat.score}/{cat.maxScore}</span>
                                            </div>
                                            <div className="h-1.5 bg-[#334155] rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full transition-all ${barColor(cat.score / cat.maxScore)}`}
